...
async def setup_nutrition(update: Update, context: CallbackContext) -> None:
    ...
    options = [
        ...
        ("Home Cooked ✅" if "Home Cooked" in user_info else "Home Cooked❔"),
...
async def ask_meal_times(update: Update, context: CallbackContext) -> None:
    ...
    await update.callback_query.message.edit_text("When’s your first and last meal?")
...
async def ask_home_cooked(update: Update, context: CallbackContext) -> None:
    context.user_data["callbackquery"] = "nutrition_home_cooked"
    keyboard = get_inline_markup([HOME_COOKED_OPTIONS])
...
async def finish_nutrition(update: Update, context: CallbackContext) -> None:
    ...
    # Move to the next section, e.g., lifestyle
    await setup_lifestyle(update, context)